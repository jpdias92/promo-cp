...
class Service:
    ...
    def __str__(self):
        return "Service: code: %s, date: %s %s, departure: %s, arrival: %s, price: %s, promo_price: %s" % \
               (self.code, calendar.day_name[self.date.weekday()], prepare_date_for_search(self.date), self.departure, self.arrival, self.price, self.promo_price)
...
def get_dates_range(n_days, applicable_weekdays):
    formatted_dates = []
...
def get_promo_next_months(web_driver, from_station, to_station, passenger_class, applicable_weekdays,
    ...
    for fd in get_dates_range(time_range, applicable_weekdays):
        web_driver.get(CP_TICKETS_PAGE)
        submit_ticket_form(web_driver, from_station, to_station, passenger_class, fd)
        services.extend(parse_search_result(web_driver.page_source, fd))
...
def main():
    ...
    parser.add_argument("-w", "--weekDay", type=int, action="append",
                        metavar="dayNumber", help="Filter results for specified weekday. \
                            Monday=0, Sunday=6. Specify desired days one by one, e.g. -w 4 -w 6")

    parser.add_argument("-r", "--timeRange", type=int,
                        metavar="dayNumber", default=TIME_RANGE,
